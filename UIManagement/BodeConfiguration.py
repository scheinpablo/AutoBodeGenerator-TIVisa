...
from UIManagement.configureMeasurement import UIConfigMeas, SweepTypes
from UIManagement.configureOsc import UIConfigOsc, ChannelTypes
from UIManagement.configureWaveGen import UIConfigWaveGen, WaveFormTypes
...
class BodeManager:
    ...
    def show_next_window(self):
        ...
        if self.iterator < (len(self.window_sequence) - 1):
            ...
            self.measure_bode(conf_osc, conf_wave, conf_meas)
            answer = messagebox.askyesnocancel(title="Mediciones completadas",
                                               message="¿Desea guardar el archivo (formato CSV)?")
            if answer:
                a = numpy.asarray(self.frequencies, self.ratio_measurements, self.phase_measurements)
                folder_path = filedialog.askdirectory()
                self.save_csv(folder_path, a, "autoMeas")
            if not answer:
                answer2 = messagebox.askyesnocancel(title="Se perderán las mediciones",
                                                    message="¿Está seguro de que desea perderlas?")
                if not answer2:
                    a = numpy.asarray(self.frequencies, self.ratio_measurements, self.phase_measurements)
                    folder_path = filedialog.askdirectory()
                    self.save_csv(folder_path, a, "autoMeas")
    ...
    def measure_bode(self, conf_osc, conf_wave, conf_meas):
        ...
        SCPI_InfiniiVision6000.write(':TRIGger:SWEep %s' % ('NORMal'))
        SCPI_InfiniiVision6000.write(':TRIGger:EDGE:SOURce %s' % ('HRESolution'))
        SCPI_InfiniiVision6000.write(':ACQuire:TYPE %s' % ('NORMal'))
        ...
        if sweep_type == SweepTypes.linearSweep.value:
            for freq in range(start_freq, stop_freq, measure_tick):
                # Signal application
            ...
            base = 10
            initial_value = start_freq
            while base < (stop_freq - start_freq):
                for freq in numpy.logspace(initial_value, base, measure_tick, base=10, dtype='int'):
                ...
                initial_value = base
                base *= 10
            for freq in numpy.logspace(initial_value, stop_freq, measure_tick, base=10, dtype='int'):
                self.measure(SCPI_33220, SCPI_InfiniiVision6000, amplitude, channel_1, channel_2,
                             current_first_range,
                             current_second_range, establishment_time, first_channel_n_, freq, second_channel_n_,
                             wave_form)
    ...
    def measure(self, SCPI_33220, SCPI_InfiniiVision6000, amplitude, channel_1, channel_2, current_first_range,
        ...
        # vertical scale
        vpp_in = SCPI_InfiniiVision6000.query_ascii_values(':MEASure:VPP %s' % (channel_1))
        vpp_out = SCPI_InfiniiVision6000.query_ascii_values(':MEASure:VPP %s' % (channel_2))
